...
const NewsAdminDashboard = () => {
  ...
  // Fetch recent articles from backend - FIXED API RESPONSE HANDLING
  const fetchRecentArticles = async () => {
    try {
      setLoading(true);
      setError(null);
      ...
      const [articlesRes, liveNewsRes, breakingNewsRes, videosRes] =
        await Promise.all([
          axios.get(`${backendURL}/api/articles`).catch(() => ({ data: { data: [] } })),
          axios.get(`${backendURL}/api/live-news`).catch(() => ({ data: { data: [] } })),
          axios.get(`${backendURL}/api/breaking-news`).catch(() => ({ data: { data: [] } })),
          axios.get(`${backendURL}/api/videos`).catch(() => ({ data: { data: [] } })),
        ]);
      ...
      const liveNewsData = Array.isArray(liveNewsRes.data) 
        ? liveNewsRes.data 
        : Array.isArray(liveNewsRes.data?.data) 
        ? liveNewsRes.data.data 
        : [];

      const breakingNewsData = Array.isArray(breakingNewsRes.data) 
        ? breakingNewsRes.data 
        : Array.isArray(breakingNewsRes.data?.data) 
        ? breakingNewsRes.data.data 
        : [];
      ...
      const combined = [
        ...
        ...liveNewsData.map((item) => ({ ...item, type: "live" })),
        ...breakingNewsData.map((item) => ({ ...item, type: "breaking" })),
